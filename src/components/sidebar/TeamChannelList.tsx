            ...
            >
              <ChevronRight
                className={`w-3.5 h-3.5 text-muted-foreground transition-transform duration-150 ${
                  expandedTeams[team.id] ? 'rotate-90' : ''
                }`}
              />
            ...
            {expandedTeams[team.id] && (
              <div className="ml-6 mt-0.5 space-y-px">
                {filteredChannels.map(channel => {
                  ...
                  const isActive = activeChatContext?.type === 'channel' && activeChatContext.id === channel.id;

                  return (
                    <button
                      key={channel.id}
                      onClick={() => dispatch(setActiveChatContext({ type: 'channel', id: channel.id }))}
